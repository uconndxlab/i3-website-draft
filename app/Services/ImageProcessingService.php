...
class ImageProcessingService
{
    ...
    /**
     * Process an uploaded tool image and generate multiple formats.
     * This just copys the original image upload for employees
     * @param UploadedFile $uploadedFile
     * @param string $directory
     * @return array Array containing paths to original, medium, and webp versions
     */
    public function processToolImage(UploadedFile $uploadedFile, string $directory = 'tool_images'): array
    {
        $originalName = pathinfo($uploadedFile->getClientOriginalName(), PATHINFO_FILENAME);
        $timestamp = time();
        ...
        // Generate medium sized version (800px width, maintain aspect ratio)
        $mediumImage = clone $image;
        $mediumImage->scaleDown(width: 800);
        Storage::disk('public')->put($mediumPath, $mediumImage->toWebp(75)->toString());

        // Generate optimized WebP version (75% quality)
        $webpImage = clone $image;
        Storage::disk('public')->put($webpPath, $webpImage->toWebp(75)->toString());
    ...
    /**
     * Delete all versions of a tool image.
     *
     * @param array $imagePaths Array of image paths to delete
     * @return void
     */
    public function deleteToolImages(array $imagePaths): void
    {
        foreach ($imagePaths as $path) {
            if ($path && Storage::disk('public')->exists($path)) {
                Storage::disk('public')->delete($path);
